...
func _ready() -> void:
	...
	assert(finnish_lines[0] == "Word Ascent · Päivän haaste", "Finnish share identifies the Daily mode")
	assert(finnish_lines[1] == "2026-07-18", "Daily share includes its puzzle date")
	assert(finnish_lines[3] == "        🟪", "Solved top row uses eight leading spaces")
	assert(finnish_lines[4] == "      🟥🟥", "Solved two-word row uses six leading spaces")
	assert(finnish_lines[5] == "    ⬜⬜⬜", "Missed three-word row uses four leading spaces")
	assert(finnish_lines[6] == "  🟩🟩🟩🟩", "Solved four-word row uses two leading spaces")
	assert(finnish_lines[7] == "⬜⬜⬜⬜⬜", "Missed five-word row keeps its five blank blocks")
	assert(finnish_lines[9] == "Tasot 3/5 · Kestävyyttä menetetty 2 · Vihjeet 1", "Finnish share summarizes levels, Stamina, and hints")
	...
	assert(unlimited_share.begins_with("Word Ascent · Endless\n\n"), "English Endless share uses its mode header without a Daily date")
	...
	assert(unlimited_share.contains("Levels 3/5 · Stamina lost 2 · Hints 1"), "English Infinity share includes its compact result summary")
