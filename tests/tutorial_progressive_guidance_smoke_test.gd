...
func _ready() -> void:
	...
	var guide_continue: Button = board.get("_tutorial_guide_button") as Button
	_assert(guide_continue != null and guide_continue.visible, "mistake explanation waits for Continue")
	guide_continue.pressed.emit()
	await get_tree().process_frame
	_assert(int(board.get("_tutorial_stage")) == 2, "hint step follows the mistake explanation")
	_assert(not (board.get("_hint") as Button).disabled, "hint button is enabled and focused")
...
func _solve_allowed_row(row_length: int) -> void:
