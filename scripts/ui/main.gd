...
func show_settings() -> void:
	...
	var sound_lab_button: Button = Button.new()
	sound_lab_button.text = "Debug: Äänilaboratorio" if str(SaveManager.settings.get("language", "en")) == "fi" else "Debug: Sound laboratory"
	sound_lab_button.custom_minimum_size = Vector2(0, 44)
	sound_lab_button.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	sound_lab_button.add_theme_font_override("font", _font_fredoka_semibold)
	sound_lab_button.add_theme_font_size_override("font_size", 14)
	sound_lab_button.add_theme_color_override("font_color", UI_TEXT)
	sound_lab_button.add_theme_stylebox_override("normal", _mode_button_style(Color("fff3bd"), UI_YELLOW))
	sound_lab_button.add_theme_stylebox_override("hover", _mode_button_style(UI_YELLOW, UI_PRIMARY))
	sound_lab_button.add_theme_stylebox_override("pressed", _mode_button_style(Color("e9c400"), UI_PRIMARY))
	sound_lab_button.pressed.connect(func() -> void: get_tree().change_scene_to_file("res://scenes/sound_lab.tscn"))
	panel.add_child(sound_lab_button)
